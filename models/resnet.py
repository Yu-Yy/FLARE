...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        # output the feature map of the last layer
        inner_feature = x.clone()
        x = self.layer4(x)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = {k: v for k, v in state_dict.items() if ("conv1" not in k) and ("fc" not in k)} # exclude the first layer and the last layer
        # It needs to verify the condition of loading
